...
{
	getWindow()->setAlwaysOnTop();//他のXcodeとかクリックしても常に最前面
	...
	// terrain
	mPerlin.setOctaves( 4 );//何重の細かいのをかけるか大きな山~小さな山で4つって意味
	mPerlinScale		= 0.1f;
	mPerlinStrength		= 10.0f;
	auto terrain		= TriMesh( geom::Plane().subdivisions( ivec2( 1000 ) ).size( vec2( 10 ) ), TriMesh::Format().positions().normals() );
	auto numVertices	= terrain.getNumVertices();
	auto positions		= terrain.getPositions<3>();//みつずつにまとめて配列化みたいな感じかな
	for( size_t i = 0; i < numVertices; i++ ) {
		positions[i].y	= noise( vec2( positions[i].x, positions[i].z ) );//noiseは上にラムダで定義してある
	}
	terrain.recalculateNormals();//変更を正規化みたいな？
    ...
    //gl::BatchRefのvectorにpushしていく(建物)
...
void TestProjectApp::update()
{
	...
	bool isAlwaysOnTop = getWindow()->isAlwaysOnTop();//Checkboxの変化を
...
void TestProjectApp::draw()
{
	...
	for( const auto &sceneObject : mScene ) {//Sceneのvectorに入ってるやつは全部描画
		sceneObject->draw();
	}
}
